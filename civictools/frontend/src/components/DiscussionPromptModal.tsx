...
export function DiscussionPromptModal({ roomId }: Props) {
  ...
  async function submit(e: Event) {
    ...
    if (cleanOptions.length < 1) return;
    setSubmitting(true);

    try {
      if (editing) {
        await pb.collection("discussion_boards").update(editing.id, {
          prompt: prompt.trim(),
        ...
        await pb.collection("discussion_boards").create({
          room: roomId,
          prompt: prompt.trim(),
          pos_x: wx,
          pos_y: wy,
