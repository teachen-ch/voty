...
export function StickyBoard({ roomId, participantId, isTeacher }: Props) {
  ...
  async function addNote(e: MouseEvent) {
    ...
    const pos_x = Math.min(
      90,
      Math.max(0, ((e.clientX - rect.left) / rect.width) * 100)
    );
    const pos_y = Math.min(
      85,
  ...
  return (
    <div>
      <p className="text-[13px] text-slate-500 mt-0 mb-2">{t("board.hint")}</p>
      <div
        ref={boardRef}
        onClick={addNote}
        className="relative w-full h-[70vh] min-h-100 bg-slate-100 border-2 border-dashed border-slate-300 rounded-lg overflow-hidden"
      >
