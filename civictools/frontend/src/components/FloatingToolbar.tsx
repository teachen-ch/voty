...
export function FloatingToolbar({ isTeacher }: Props) {
  ...
  async function confirmLockRoom() {
    ...
    try {
      const updated = await pb
        .collection("rooms")
        .update(room.id, { interactions_locked: true });
      currentRoom.value = updated;
      setConfirmLock(false);
    } catch (error) {
      console.error("Failed to lock room interactions", error);
    }
  }
  ...
  return (
    <>
      ...
      {confirmLock && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-xl p-7 w-[min(90vw,32rem)] flex flex-col gap-5"
