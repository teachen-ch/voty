...
export function MobileRoomView({
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto bg-slate-50">
      <div className="flex flex-col gap-3 p-3 pt-20 pb-24 max-w-2xl mx-auto">
      ...
      {isTeacher && (
        <div className="fixed bottom-0 left-0 right-0 flex gap-2 p-3 bg-white border-t border-slate-200 safe-area-inset-bottom">
          <button
            onClick={() => (discussionModal.value = {})}
            className="flex-1 px-3 py-2 rounded-lg bg-white border border-slate-200 text-sm text-slate-700 hover:bg-slate-100"
          >
            + Discussion
          </button>
          <button
