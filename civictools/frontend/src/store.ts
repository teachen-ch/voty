...
export function getParticipantName(participantId: string | null | undefined): string {
...
export function applyNoteEvent(action: string, record: RecordModel) {
  if (action === 'create') {
    ...
    if (pid && !participantCache.value.has(pid)) {
      pb.collection('participants').getOne(pid).then(p => cacheParticipants([p]))
    }
  } else if (action === 'update') {
    stickyNotes.value = stickyNotes.value.map(n => (n.id === record.id ? record : n))
