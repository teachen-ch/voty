  ...
  return (
    <div class="page">
      <div class="flex justify-between items-center mb-6">
        ...
        <button class="secondary" onClick={logout}>{t('dashboard.logout')}</button>
      </div>

      <form onSubmit={createRoom} class="flex gap-2 mb-6">
        <input
          value={roomName}
      ...
      <div class="flex flex-col gap-[10px]">
        {rooms.map(room => (
          <div key={room.id} class="card flex items-center gap-3">
            <span class="flex-1 font-medium">{room.name}</span>
            <ShareLink roomId={room.id} />
            <button onClick={() => navigate(`/room/${room.id}`)}>{t('dashboard.open')}</button>
          </div>
        ))}
      </div>
    </div>
