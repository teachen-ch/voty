...
export function Join({ roomId }: Props) {
  ...
  const [nickname, setNickname] = useState(studentSession.value?.nickname ?? '')
  ...
  if (notFound) {
    return (
      <div class="page max-w-[400px]">
        <div class="card">
          <p class="error">{t('join.notFound')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div class="page max-w-[400px]">
      <h1>{t('appName')}</h1>
      <div class="card">
        <h2>{t('join.title')}{room ? `: ${room.name}` : '…'}</h2>
