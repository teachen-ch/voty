...
export default function Projekt(): React.ReactElement {
  return (
    <AppPage heading="voty.ch – Ein Projekt zur Förderung der Demokratie an den Schulen">
      ...
      <Text mt={4}>
        Wir glauben, dass ein demokratisches Verständnis nur durch demokratische
        Praxis erreicht werden kann. Deshalb möchten wir mit voty.ch bereits in
        der Grundschule eine Basis für das Demokratieverständnis schaffen,
        dieses mit den Schüler*Innen praktisch anwenden und so die Fähigkeiten
        fördern, welche für eine demokratische Partizipation wichtig sind.
      </Text>
      <Card fontSize={2}>
        <Heading as="h2" mt={0}>
        ...
        <Text mt={4}>
          <strong>Modul 1: «Demokratie verstehen»</strong>{" "}
        ...
        <Text mt={4}>
          <strong>Modul 2: «Demokratie testen»</strong>{" "}
          <Tag bg={"secondary"}>Live</Tag>
          <br /> Ein Abstimmungsmodul, mit dem aktuelle nationale Urnengänge von
          den Schüler*Innen in der Klasse mit der Lehrerschaft diskutiert und
          danach durchgeführt werden können.
          <Detail>
            <Text my={3}>
        ...
        <Text mt={4}>
          <strong>Modul 3: «Demokratie erleben»</strong>{" "}
      ...
      <Text>
        Sämtliche Angebote von voty.ch werden Schulen und Lehrpersonen kostenlos
        angeboten und richten sich primär an die Stufen Sek-1, Gymnasium und
        Berufsschule. Die Plattform voty.ch mit den drei Modulen ist
        webbasierend aufgebaut und kann auf jedem Computer, Tablet oder
      ...
      <Text fontSize={2} lineHeight="2em">
        <ul>
          <li>
            <strong>Stefan Niederhauser</strong> (
            <A href="https://linkedin.com/in/sniederhauser" variant="underline">
              LinkedIn
            </A>
      ...
      <Text>
        Sie sind am Projekt interessiert, haben Fragen, Anregungen oder möchten
        als Pilotklasse am Projekt teilnehmen? Wir freuen uns auf Ihre
        Kontaktaufnahme: info@teachen.ch
      </Text>
...
const Tag: React.FC<{ bg?: string }> = ({ children, bg }) => (
  <Text
    fontSize={1}
    bg={bg || "gray"}
    sx={{ borderRadius: 20, display: "inline" }}
    color="white"
    px={3}
    py={1}
    mx={2}
